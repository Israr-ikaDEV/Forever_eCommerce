...
const Collection = () => {
  const { listProduct, search, showSearch } = useContext(ShopContext);
  ...
  const applyFilter = () => {
    if (!listProduct || listProduct.length === 0) return; // Ensure products are available

    let productsCopy = listProduct.slice(); // Create a shallow copy of products
  ...
  useEffect(() => {
    applyFilter();
  }, [category, subCategory, search, showSearch, listProduct]);
